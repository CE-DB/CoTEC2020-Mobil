...
public class ContactPathologiesAdapter extends RecyclerView.Adapter<ContactPathologiesAdapter.PathologyViewHolder> {


    private List<ContactsQuery.Pathology> pathologies;

    public ContactPathologiesAdapter(List<ContactsQuery.Pathology> pathologies) {
    ...
    public PathologyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new ContactPathologiesAdapter.PathologyViewHolder(listItem);
    }

    @SuppressLint("SetTextI18n")
    @Override
    public void onBindViewHolder(@NonNull PathologyViewHolder holder, int position) {
        final ContactsQuery.Pathology p = pathologies.get(position);
